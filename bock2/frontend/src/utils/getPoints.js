...
function quadraticBezierCurve(p1, p2, controlPoint, numPoints) {
    ...
    for (let i = 0; i < numPoints; i++) {  
        let t = i / (numPoints - 1); // Ensures t always reaches exactly 1
        ...
        points.push({ latitude: y, longitude: x }); // Correct order
    }
...
const calculateControlPoint = (p1, p2) => {
    ...
    const d = Math.sqrt((p2[0] - p1[0]) ** 2 + (p2[1] - p1[1]) ** 2) + epsilon;
    const scale = 1;
    const h = d * scale;
    const w = d / 2;
    const x_m = (p1[0] + p2[0]) / 2;
...
export const getPoints = (places) => {
    if (!places || places.length < 2) {
        console.error("Error: 'places' must contain at least two points.");
        return [];
    }
